...
export const loadListMedia = async (page: number, perPage = 15, type: string, status: string[], sort: string): Promise<{
  listItems: MediaListReturn
}> => {
  const { load } = getListMedia(type, sort, page, perPage, status);
